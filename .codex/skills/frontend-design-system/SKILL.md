...
14. Form controls have an explicit shared contract: `Input`, `InputGroup`, `Textarea`, `SelectTrigger`, `CommandInput`, and combobox field shells use `--app-input-radius`; compound controls keep inner inputs square. Use `--app-form-field-gap`, `--app-form-related-gap`, and `--app-form-section-gap` for label, related-action, and section rhythm. A secondary recovery link may be visually small but must retain a 44px hit area. In a hard-gated credential flow, place Recovery key beside Sign out in the same quiet escape group.
